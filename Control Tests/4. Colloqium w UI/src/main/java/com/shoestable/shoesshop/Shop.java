...
public class Shop {
    ...
    public void outBoots(String file_name) throws IOException {
        FileWriter fileWriter = new FileWriter(file_name);
        fileWriter.write(Container.size() + "\n");
        for (int i = 0; i < Container.size(); i++) {
            fileWriter.write("     \"" + Container.get(i).getName() + "\"\t      (" + Container.get(i).getMark() + ")      \t" +
                    Container.get(i).getDate() + "    \t   " + Container.get(i).getPrice() + "\t\t  " + Container.get(i).getCount() + "\n");
        }
        fileWriter.close();
    }
    public ArrayList<Boots> getShop(){
        return Container;
    }
    ...
    public int binarySearch(String to_search) throws IOException {
        //FileWriter fileWriter = new FileWriter(file_name, true);
        Boots boots = new Boots();
